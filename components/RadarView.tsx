...
const RadarView: React.FC<Props> = ({ allCompetencies }) => {
  ...
  const getMaturityColor = (level: MaturityLevel) => {
    switch (level) {
      case 'Adopt': return 'text-blue-600 dark:text-blue-400';
      case 'Trial': return 'text-cyan-600 dark:text-cyan-400';
      case 'Assess': return 'text-indigo-600 dark:text-indigo-400';
      case 'Experimental': return 'text-slate-500 dark:text-slate-400';
    }
  };

  const getMaturityStyle = (level: MaturityLevel) => {
    switch (level) {
      case 'Adopt': return 'text-blue-600 border-blue-500/30 bg-blue-500/5 dark:text-blue-400';
      case 'Trial': return 'text-cyan-600 border-cyan-500/30 bg-cyan-500/5 dark:text-cyan-400';
      case 'Assess': return 'text-indigo-600 border-indigo-500/30 bg-indigo-500/5 dark:text-indigo-400';
      case 'Experimental': return 'text-slate-500 border-slate-500/30 bg-slate-500/5 dark:text-slate-400';
    }
  };
  ...
  return (
    <div className="h-full flex flex-col overflow-y-auto no-scrollbar bg-slate-50 dark:bg-[#030712] transition-colors duration-300 scroll-smooth">
      {/* Radar Section */}
      <div className={`flex-none w-full transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col items-center justify-center bg-blue-500/[0.02] dark:bg-blue-500/5 border-b border-slate-200 dark:border-white/5 relative ${
        selectedQuadrant 
          ? 'h-[50px] md:h-[60px] overflow-hidden' 
          : 'h-[360px] md:h-[70vh] py-4'
      }`}>
        {selectedQuadrant && (
          <>
            <div className="absolute left-6 top-1/2 -translate-y-1/2 z-30">
              <button 
                ...
                className="p-2 rounded-lg bg-white dark:bg-slate-900/80 backdrop-blur-sm text-slate-900 dark:text-white hover:bg-blue-600 hover:text-white transition-all active:scale-95 shadow-xl border border-slate-200 dark:border-white/10 group flex items-center justify-center"
                title="Back to Full Radar"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" className="transition-transform group-hover:-translate-x-1"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
              </button>
            </div>
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none text-center z-10">
              <h2 className="text-xs md:text-sm font-black text-slate-400 dark:text-white/40 uppercase tracking-[0.6em] animate-in fade-in duration-700 whitespace-nowrap">
                {selectedQuadrant}
              </h2>
            </div>
          </>
        ...
        <div className={`w-full h-full max-w-6xl flex items-center justify-center transition-opacity duration-500 ${selectedQuadrant ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
           <TechnicalRadar 
      ...
      {/* Filter & Action Bar */}
      <div className="sticky top-0 z-40 bg-white/80 dark:bg-[#030712]/80 backdrop-blur-md border-b border-slate-200 dark:border-white/5 px-4 md:px-6 py-4">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-80">
            <input 
              ...
              className="w-full bg-slate-100 dark:bg-[#0f172a] border border-slate-200 dark:border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-[14px] font-medium focus:ring-1 focus:ring-blue-500/50 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-600 text-slate-900 dark:text-white"
            />
          ...
          <div className="flex items-center gap-6">
            ...
            >
              <span className="text-[12px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-500 group-hover:text-slate-900 dark:group-hover:text-slate-300 transition-colors">
                Maturity Definitions
              </span>
              <div className={`transition-transform duration-300 ${showLegend ? 'rotate-180' : ''}`}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" className="text-slate-400 dark:text-slate-600"><path d="M6 9l6 6 6-6"/></svg>
              </div>
            </button>
            <div className="text-[12px] font-bold text-blue-600 uppercase tracking-widest bg-blue-600/10 px-3 py-1.5 rounded-lg border border-blue-600/20">
      ...
      <div className={`flex-none w-full bg-white dark:bg-[#030712] border-b border-slate-200 dark:border-white/5 transition-all duration-300 overflow-hidden ${showLegend ? 'max-h-96 opacity-100 py-6' : 'max-h-0 opacity-0'}`}>
        <div className="max-w-[1400px] mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {MATURITIES.map(m => (
            <div key={m} className="space-y-1">
              <h5 className={`text-[12px] font-black uppercase tracking-widest ${getMaturityColor(m)}`}>
                {m}
              </h5>
              <p className="text-[13px] text-slate-500 dark:text-slate-500 font-medium leading-relaxed">
                {MATURITY_DESC[m]}
              </p>
            </div>
          ))}
        ...
        <div className="max-w-[1400px] mx-auto w-full">
          {filteredItems.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
              <div className="w-16 h-16 bg-white dark:bg-[#0f172a] rounded-2xl flex items-center justify-center text-slate-300 dark:text-slate-700 border border-slate-200 dark:border-white/5">
              ...
              <div>
                <h4 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight">No Results Found</h4>
              ...
              <button 
                ...
                className="text-sm font-black text-blue-600 dark:text-blue-500 uppercase tracking-widest hover:text-slate-900 dark:hover:text-white transition-colors"
              >
                Clear all filters
              </button>
            </div>
      ...
      {selectedCompetency && (() => {
        ...
        return (
          <div className="fixed inset-0 z-[100] flex flex-col bg-slate-900/40 dark:bg-[#030712]/95 backdrop-blur-xl p-4 animate-in fade-in">
             <div className="flex-1 flex flex-col bg-white dark:bg-[#0f172a] w-full max-w-3xl mx-auto shadow-2xl overflow-hidden md:my-10 rounded-[2.5rem] border border-slate-200 dark:border-white/10 relative">
                
                <div className="flex-none px-8 py-10 md:px-14 flex justify-between items-start border-b border-slate-100 dark:border-white/5 bg-gradient-to-b from-slate-50 dark:from-white/[0.02] to-transparent">
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`px-2.5 py-1 rounded-lg border text-[12px] font-black uppercase tracking-widest ${getMaturityColor(selectedCompetency.maturity)} border-current bg-white dark:bg-white/5`}>
                        {selectedCompetency.maturity}
                      </span>
                      <span 
                        className="px-2.5 py-1 rounded-lg border text-[12px] font-black uppercase tracking-widest"
                        style={{ borderColor: `${catColor}44`, color: catColor, backgroundColor: `${catColor}11` }}
                      >
                        {selectedCompetency.category}
                      </span>
                    </div>
                    <h2 className="text-2xl md:text-5xl font-black text-slate-900 dark:text-white uppercase tracking-tighter leading-none">{selectedCompetency.name}</h2>
                  </div>
                  <button 
                    onClick={() => setSelectedCompetency(null)} 
                    className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-500 hover:text-red-600 dark:hover:text-white dark:hover:bg-red-600 transition-all border border-slate-200 dark:border-white/10 shadow-lg"
                  >
                ...
                <div className="flex-1 overflow-y-auto px-8 md:px-14 pb-14 space-y-12 no-scrollbar mt-10">
                  <section className="space-y-6">
                    <h4 className="text-[14px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] border-l-4 border-blue-600 dark:border-blue-600 pl-4">Executive Brief</h4>
                    <p className="text-xl md:text-3xl font-medium text-slate-800 dark:text-slate-100 leading-[1.4] whitespace-pre-wrap">{definition}</p>
                  </section>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="p-8 rounded-[2rem] bg-blue-600/[0.03] dark:bg-blue-600/5 border border-blue-600/10 dark:border-blue-600/10 space-y-4 shadow-inner">
                      <h4 className="text-[13px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-blue-600 dark:bg-blue-500 animate-pulse"></span>
                        Architectural Context
                      </h4>
                      <p className="text-[16px] text-slate-600 dark:text-slate-400 font-medium leading-relaxed">{techData.application || "Critical standard for modern enterprise-grade AI systems and production infrastructure."}</p>
                    </div>
                    <div className="p-8 rounded-[2rem] bg-slate-100/50 dark:bg-slate-500/5 border border-slate-200 dark:border-slate-500/10 space-y-4 shadow-inner">
                      <h4 className="text-[13px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
